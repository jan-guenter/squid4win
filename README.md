...
parse. The helper therefore normalizes both the Windows service command line to
include `-f <config>` and the registry-backed `ConfigFile`/`CommandLine`
values so startup and spawned Squid processes do not fall back to Squid's
compiled defaults. Because upstream service startup still splits the stored
registry `CommandLine` on whitespace without quote support, the install root
