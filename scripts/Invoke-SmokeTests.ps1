...
$buildProfile = Get-Content -Raw -LiteralPath $resolvedBuildProfilePath | ConvertFrom-Json
...
$runtimeDlls = @($buildProfile.runtimeDlls | ForEach-Object { [string]$_ })
if ($runtimeDlls.Count -eq 0) {
    throw "build-profile.json must declare runtimeDlls so smoke tests can validate the staged native runtime payload."
}
...
$summaryLines = @(
    ...
    ('- Runtime DLLs: `{0}`' -f ($runtimeDlls -join ', ')),
