...
Use this skill when the task is about **consuming** packages with Conan 2
rather than authoring a package recipe from scratch.
...
- deciding between Conan and a different package manager; use a comparison
  skill if one is available

## Working method

1. Inspect the current consumer state first.
   - Read the existing Conan manifest, profiles, lockfiles, build scripts,
...
2. Choose the right manifest shape.
   ...
   - Use `conanfile.py` when you need conditional requirements, custom
     layout, validation, custom `generate()` logic, or build-system
     orchestration.
3. Treat profiles as the source of truth for real builds.
   - `conan profile detect --force` is acceptable for first-run local
     bootstrap.
   - For CI, release, and team workflows, prefer explicit checked-in
     profiles or configuration installed through `conan config install`.
...
4. Prefer the explicit Conan-plus-build-system flow.
   ...
   - For CMake, prefer `CMakeToolchain` plus `CMakeDeps` unless the
     project has a deliberate reason to use another integration.
   - Avoid implicit `cmake-conan` flows unless the project explicitly
     needs that IDE-driven behavior.
5. Use dependency traits correctly.
   ...
   - Use generated build and run environments when tools or shared
     libraries need activation.
6. Model settings, options, and environments deliberately.
   ...
   - When consuming shared libraries, activate the generated run
     environment so executables can resolve dynamic libraries at runtime.
7. Use version ranges intentionally.
   ...
   - If ranges are in use and newer remote versions should be preferred
     over the cache, use `--update`.
8. Use lockfiles for reproducibility.
   ...
   - Use `--lockfile-partial` only when intentionally extending an
     incomplete lock.
9. Manage remotes conservatively.
   - Inspect the current remote configuration before adding or changing
     remotes.
   - Prefer organization-controlled remotes for production workflows.
   - Protected repositories should generally be writable by CI rather than
     by individual developer workstations.
10. Troubleshoot through Conan's binary model.
    ...
    - Graph drift: inspect version ranges, revisions, remotes, `--update`,
      and lockfile usage.
...
- use at least one simple consumer prompt and one cross-build or lockfile
  prompt
- treat Conan 1 guidance, shared-cache concurrency assumptions, or blind
  `--build=missing` advice as failures
- verify that profiles, host/build separation, and lockfile intent are
  explained before ad-hoc fixes are suggested
- verify that `conanfile.txt` versus `conanfile.py` advice matches the
  actual complexity of the consumer

## Sources

- Conan 2 docs:
  - [The flexibility of conanfile.py](https://docs.conan.io/2/tutorial/consuming_packages/the_flexibility_of_conanfile_py.html)
